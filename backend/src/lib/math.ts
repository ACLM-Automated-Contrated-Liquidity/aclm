...
import { Pool, Position, Tick, Token } from "../interfaces/uniswap.interface"
...
export const calculatePositionFees = (
    ...
    let tickLowerFeeGrowthOutside_0 = new bn(position.tickLower.feeGrowthOutside0X128)
    let tickLowerFeeGrowthOutside_1 = new bn(position.tickLower.feeGrowthOutside1X128)

    // Fee growth outside '𝑓𝑜' of our upper tick for both token 0 and token 1
    let tickUpperFeeGrowthOutside_0 = new bn(position.tickUpper.feeGrowthOutside0X128)
    let tickUpperFeeGrowthOutside_1 = new bn(position.tickUpper.feeGrowthOutside1X128)
    ...
    let uncollectedFees_0 = mulDiv(liquidity, fr_t1_0.minus(feeGrowthInsideLast_0), Q128)
    let uncollectedFees_1 = mulDiv(liquidity, fr_t1_1.minus(feeGrowthInsideLast_1), Q128)

    // Decimal adjustment to get final results
    let uncollectedFeesAdjusted_0 = uncollectedFees_0.div(
        expandDecimals(1, Number(token0?.decimals || 18)).toFixed(Number(token0?.decimals || 18))
    )
    let uncollectedFeesAdjusted_1 = uncollectedFees_1.div(
        expandDecimals(1, Number(token1?.decimals || 18)).toFixed(Number(token1?.decimals || 18))
    )

    return [uncollectedFeesAdjusted_0.toNumber(), uncollectedFeesAdjusted_1.toNumber()]
}
...
export const getPositionTokensDepositRatio = (P: number, Pl: number, Pu: number): number => {
...
export const getTokenAmountsFromDepositSimple = (
    ...
    const deltaL = depositAmountUSD / (2 * Math.sqrt(P) - Math.sqrt(Pl) - P / Math.sqrt(Pu))
