...
contract LiquidityExamples is IERC721Receiver {
    ...
    // address internal immutable v3pool;
    ...
    {
        console.log("Minting a new position for DAI / WETH with amounts: %s ; %s", amount0ToMint, amount1ToMint);
        ...
        TransferHelper.safeApprove(WETH, nonfungiblePositionManager, amount1ToMint);
        ...
        console.log("Minted position: %s, %s, %s, %s", tokenId, amount0, amount1);
        ...
        if (amount0 < amount0ToMint) {
            ...
            TransferHelper.safeApprove(DAI, nonfungiblePositionManager, refund0);
            TransferHelper.safeTransfer(DAI, msg.sender, refund0);
        }

        if (amount1 < amount1ToMint) {
            uint256 refund1 = amount1ToMint - amount1;
            TransferHelper.safeApprove(WETH, nonfungiblePositionManager, refund1);
    ...
    //     UniswapV3Pool(poolAddr).mint(msg.sender, tickLower, tickUpper, amount, data);
    // }
    ...
    //     bytes calldata
    // ) external {
    //     console.log("Mint callback: %s; %s", amount0Owed, amount1Owed);
    //         emit MintCallback(amount0Owed, amount1Owed);
    //         (bool success, bytes memory data) = UniswapV3Pool(msg.sender).token0().call(abi.encodeWithSelector(IERC20.approve.selector, msg.sender, amount0Owed));
    //         require(success && (data.length == 0 || abi.decode(data, (bool))), 'SA');

    //         (success, data) = UniswapV3Pool(msg.sender).token1().call(abi.encodeWithSelector(IERC20.approve.selector, msg.sender, amount1Owed));
    //         require(success && (data.length == 0 || abi.decode(data, (bool))), 'SA');
    //         if (amount0Owed > 0)
    //             IERC20(UniswapV3Pool(msg.sender).token0()).transferFrom(address(this), msg.sender, amount0Owed);
    //         if (amount1Owed > 0)
    //             IERC20(UniswapV3Pool(msg.sender).token1()).transferFrom(address(this), msg.sender, amount1Owed);
    // }
        ...
        )
    {
        ...
        // TransferHelper.safeApprove(token0, nonfungiblePositionManager, amountAdd0);
        // TransferHelper.safeApprove(token1, nonfungiblePositionManager, amountAdd1);
